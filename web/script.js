...
jsonData.calltree.forEach(caller => {
    ...
    if (!moduleNodes[baseName]) {
        ...
        nodes.add({
            ...
            group: 'module',
            shape: 'box',
            color: { background: '#f8f9fa', border: '#4a90e2', highlight: { border: '#4a90e2' } },
    ...
    if (!nodes.get(callerUniqueId)) { // Check if node already exists
        nodes.add({
            ...
            group: 'function',
            shape: 'box',
            color: { background: '#e6f3ff', border: '#4a90e2', highlight: { background: '#d1e8ff', border: '#4a90e2' } },
            font: { size: 12 },
            size: 30,
